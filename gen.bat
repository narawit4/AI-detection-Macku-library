...
if errorlevel 1 exit /b %errorlevel%
python -m nuitka --onefile --mingw64 --assume-yes-for-downloads --progress-bar=none --windows-console-mode=disable --enable-plugin=tk-inter --output-filename=Jitter.exe --output-dir=build-output main.py > build-output\build.log 2>&1
if errorlevel 1 (
